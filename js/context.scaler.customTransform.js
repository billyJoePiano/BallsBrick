...
(function initiateRescalingOfCanvasContexts(canvasToFitToScreen) {
    const modifyArgs = { //true indicates multiply the argument(s) by m
        moveTo: true, //simple boolean true means multiply all numerical finite values
        lineTo: true,
        arc: [true, true, true], //array indicated which arguments to multiply... in this case don't transform arc radians (start/finish) or counterclockwise flag (4th, 5th, 6th arguments, cardinally speaking)
        clearRect: true,
        fillRect: true,
        strokeRect: true,
        strokeText: [false, true, true, true],
        fillText: [false, true, true, true],
        setLineDash: (m, [dashArray]) => [dashArray.map(arg => arg * m)], //enclosed as array for arguments
        createLinearGradient: true,
        drawImage: function (m, args) {
            //depends on number of args
            //??? not controlling for out-of-bound source coordinates, to prevent Safari from failing here... relying on caller to do this
            let i;
            if (args[0] instanceof HTMLCanvasElement.OffscreenCanvas || args[0] instanceof HTMLCanvasElement) {
                i = 1;
        ...
        //getLineDash: () => Array.prototype.map.call(args, arg => arg / m)
    }

    function getMeasureTextFunction(nativeFontSymbol_accessorCallback) {
        let symbol; // will be set on first invocation of the function through the callback function.
                    // The symbol does not yet exist when this function is first submitted to ObjectRescaler
        return measureText;

        function measureText(m, nativeFunction, ...args) {
            let actualFont = this[symbol ?? (symbol = nativeFontSymbol_accessorCallback())];
            let apparentFont = this.font;
            this[symbol] = apparentFont;

            let textMetric = nativeFunction.apply(this, args);

            this[symbol] = actualFont;
            return textMetric;
        }
    }

    const embedIn = { //a function which will invoke the native using custom behavior
        // wrapper function will pass native function as the second argument to this, followed by the args passed in by the caller
        //measureText will go here
    }

    const modifyReturn = {
        //simple boolean true indicates divide the return value by m
        //otherwise a function which will accept the native return value as second argument, and return the transformed value
        getLineDash: (m, dashArray) => dashArray.map(arg => arg / m)
    }

    const gettersSetters = { // true boolean indicates multiply for setter and divide for getter. Get/set functions (inside object) for custom behavior
        /*lineWidth: { //I want lines to be thinner
            get: (m, width) => width / m * 2,
            set: (m, width) => width * m / 2,
        }*/
        font: false,  //this is only for the native accessor symbol
        lineWidth: true,
    }

    function onMultiplierChange(newMultiplier, oldMultiplier, toggleToNew, toggleToOld) {
        multiplier = newMultiplier;
        offscreenRescaler.set(newMultiplier);
        for (let canvas of document.getElementsByTagName("canvas")) {
            for (let offscreen of canvas.offscreenCanvases) {
                updateScale(offscreen);
                offscreen.redraw?.(offscreen.context);
            }
            updateScale(canvas);
            ...
            function updateScale(canvas) {
                ...
                let oldLineDash = context.getLineDash();
    ...
    function checkForNewCanvasElements(canvases, invokeDoAutoResize) {
        for (let canvas of canvases) {
            ...
            if (!(widthDescriptor && heightDescriptor)) {
                ...
                canvas.baselineFont = canvas.getContext("2d")[canvasRescaler.symbols.font];
                installInstanceSpecificGettersSetters(canvas, false, invokeDoAutoResize);
    ...
    function installInstanceSpecificGettersSetters(canvas, isOffscreen = false, invokeDoAutoResize = !isOffscreen) { //these are specific to each canvas/context... not for the prototype
        ...
        let baselineDimensions = { width: canvas.width, height: canvas.height };
        let offscreenArray = [];
        ...
        let properties = {
            ...
            addOffscreen: {
                value: function addOffscreen() {
                    ...
                    installInstanceSpecificGettersSetters(newOffscreenCanvas, true);
        ...
        while (!nativeFont) {
            //find the native getters/setter, walk up the proto chain if neccessary
            proto = Object.getPrototypeOf(proto);
            nativeFont = Object.getOwnPropertyDescriptor(proto, "font");
        }

        let baselineFont = context.font;
        let lastNative = baselineFont;

        let fontDescriptor = {
            get: function () {
                return nativeFont.get.call(context).replace(/(\d+.\d+|\d+)/, (match) => Number(match) / multiplier);
            },
            set: function (value) {
                if (value === useBaseline) value = baselineFont;
                nativeFont.set.call(context, value?.replace?.(/(\d+.\d+|\d+)/, (match) => Number(match) * multiplier));
                if (lastNative !== (lastNative = nativeFont.get.call(context))) baselineFont = value; //testing whether the new font 'took', and re-assigning the baseline if it did
            }
        };
        Object.defineProperty(context, "font", fontDescriptor);
        ...
        function invokeDeferredAutoResizeAndRedraw() {
            if (canvas === canvasToFitToScreen) {
                if (pendingResizeOfCanvasToFitToScreen) return; //breaks an infinite loop from adjustCanvasSize -> onMultiplierChange -> canvas.width -> invokeDeferredAutoResizeAndRedraw -> etc
            ...
            function invokeAutoResizeAndRedraw() {
                ...
                if (pendingResizeOfCanvasToFitToScreen) {
                    if (canvas === canvasToFitToScreen) {
                        ...
                        return setTimeout(invokeAutoResizeAndRedraw, loopbackTracker * 10);
    ...
    function adjustCanvasSizes(event) {
        ...
        let newMultiplier = Math.max(window.devicePixelRatio, cssScale, getBrowserZoom(), 1);
        if (newMultiplier !== multiplier) {
            canvasRescaler.set(newMultiplier);
            //this indirectly invokes the "onMultiplierChange" event which triggers resize of drawing scales, css sizes, and deferred redraw of all canvases

        } else {
            onMultiplierChange(multiplier, multiplier, () => { }, () => { });
            // if the multiplier doesn't change, it is still possible to have change in the cssScale requiring a canvas resizing
            // but onMultiplierChange wouldn't be called unless there is a change in multiplier... so invoking manually instead
        }

        canvasToFitToScreen.redraw?.();
        ...
        setTimeout(setCSSdisplayStyle);

        function setCSSdisplayStyle() {
            if (pendingResizeOfCanvasToFitToScreen) {
                if (++loopbackTracker < 16) {
                    return setTimeout(setCSSdisplayStyle, loopbackTracker * 10);
    ...
    function adjustResolutionForBrowserZoom() {
        canvasRescaler.set(Math.max(window.devicePixelRatio, cssScale, getBrowserZoom(), 1));
    ...
    let offscreenRescaler;

    try {
        embedIn.measureText = getMeasureTextFunction(() => offscreenRescaler.symbols.font);

        if (void window.OffscreenCanvas) {
            HTMLCanvasElement.OffscreenCanvas = window.OffscreenCanvas;
            offscreenRescaler = new ObjectRescaler(OffscreenCanvasRenderingContext2D.prototype, modifyArgs, embedIn, modifyReturn, gettersSetters, multiplier);
            //same as standard context rescaler below, except for no onMultiplierChange callback, since the offscreens (attached to an on-screen) are dealt with by the main callback
        ...
        offscreenRescaler = {
            set: () => { /*do nothing... it's already using the rescaler for CanvasRenderingContext2D.prototype*/ },
        }
    ...
    for (let canvas of document.getElementsByTagName("canvas")) {
        installInstanceSpecificGettersSetters(canvas);
    }

    embedIn.measureText = getMeasureTextFunction(() => canvasRescaler.symbols.font);

    const canvasRescaler = new ObjectRescaler(CanvasRenderingContext2D.prototype, modifyArgs, embedIn, modifyReturn, gettersSetters, multiplier, onMultiplierChange);
    Object.defineProperty(CanvasRenderingContext2D, "multiplier", { get: canvasRescaler.get, set: canvasRescaler.set, configurable: false, enumerable: true });

    if (!"symbols" in offscreenRescaler) offscreenRescaler.symbols = { font: canvasRescaler.symbols.font };
...
*
* NOTE: If a property has more than one function/getter/setter substitution, the substitutes will be nested in the order they are explained above.
